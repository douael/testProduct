...
<div itemscope itemtype="https://schema.org/Product">
	...
	<div class="primary_block row">
		...
		<div class="pb-left-column col-xs-12">
			<!-- product img-->
			<div id="image-block"  width="100%"class="clearfix">
				{if $product->new}
					<span class="new-box">
				...
				{/if}
				{if $product->on_sale}
					<a class="sale-box no-print">
						<!--<img class="promo" src="../themes/default-bootstrap/img/icons/promo.png">-->
				...
				{if $have_image}
					<span id="view_full_size">
						...
						{if isset($images) && count($images) > 1}
								<a id="view_left" class="" title="{l s='Other views'}" >
									{l s='Previous'}
								</a>
						{/if}
							<img id="bigpic" itemprop="image" src="{$link->getImageLink($product->link_rewrite, $cover.id_image, 'large_default')|escape:'html':'UTF-8'}" title="{if !empty($cover.legend)}{$cover.legend|escape:'html':'UTF-8'}{else}{$product->name|escape:'html':'UTF-8'}{/if}" alt="{if !empty($cover.legend)}{$cover.legend|escape:'html':'UTF-8'}{else}{$product->name|escape:'html':'UTF-8'}{/if}" width="{$largeSize.width}" height="{$largeSize.height}"/>
							{if isset($images) && count($images) > 1}
							<a id="view_right" title="{l s='Other views'}" >
								{l s='Next'}
							</a>
							{/if}
							{*if !$content_only}
		...
		<!-- end center infos-->
		<!-- pb-right-column-->
		<div class="pb-right-column col-xs-4 col-sm-6 custom_sheetDescr1">
				...
				<div class="box-info-product">
					<div class="content_prices clearfix">
						{if $product->show_price && !isset($restricted_country_mode) && !$PS_CATALOG_MODE}
							...
							<div>
								...
								{/strip}</p>
								{else}
								<p class="our_price_display_reduction" itemprop="offers" itemscope itemtype="https://schema.org/Offer">{strip}
									{if $product->quantity > 0}<link itemprop="availability" href="https://schema.org/InStock"/>{/if}
									{if $priceDisplay >= 0 && $priceDisplay <= 2}
										<span id="our_price_display_reduction" class="price" itemprop="price" content="{$productPrice}">{convertPrice price=$productPrice|floatval}</span>
										<meta itemprop="priceCurrency" content="{$currency->iso_code}" />
										{hook h="displayProductPriceBlock" product=$product type="price"}
										{/if}
								{/strip}</p>
					...
					{if ($display_qties == 1 && !$PS_CATALOG_MODE && $PS_STOCK_MANAGEMENT && $product->available_for_order)}
						...
						<p id="availability_statut"{if !$PS_STOCK_MANAGEMENT || ($product->quantity <= 0 && !$product->available_later && $allow_oosp) || ($product->quantity > 0 && !$product->available_now) || !$product->available_for_order || $PS_CATALOG_MODE} style="display: none;"{/if}>
							...
							<span id="availability_value">{if $product->quantity <= 0}{if $PS_STOCK_MANAGEMENT && $allow_oosp}{$product->available_later}{else}{l s='This product is no longer in stock'}{/if}{elseif $PS_STOCK_MANAGEMENT}{l s="In stock"}{/if}</span>
							<span id="quantityAvailable">{$product->quantity|intval}</span>
								<span {if $product->quantity > 1} style="display: none;"{/if} id="quantityAvailableTxt">{l s='qt.'}</span>
								<span {if $product->quantity == 1} style="display: none;"{/if} id="quantityAvailableTxtMultiple">{l s='qt.'}</span>
						</p>
											...
											<div class="attribute_list">
												{if ($group.group_type == 'select')}
													<select name="{$groupName}" id="group_{$id_attribute_group|intval}" class="form-control attribute_select no-print">
															<option value="" selected disabled hidden>{$group.name|escape:'html':'UTF-8'}&nbsp;</option>
					...
					<div class="box-cart-bottom">
						<div{if (!$allow_oosp && $product->quantity <= 0) || !$product->available_for_order || (isset($restricted_country_mode) && $restricted_country_mode) || $PS_CATALOG_MODE} class="unvisible"{/if}>
							<p id="add_to_cart" class="buttons_bottom_block no-print">
								<button type="submit" name="Submit" class="btn btn-default btn-default1">
						...
						<div class="text_complementaire">
							{if (Module::isInstalled(productdescr)) && Module::isEnabled(productdescr)}
							{hook h='myCustomHookPreOrders'}
							{/if}
						</div>
						<div class="text_complementaire " id="fabrication">
							<span data-toggle="collapse" data-target="#demo1">{l s="Fabrication à la demande"}</span>
							<div id="demo1" class="collapse">
								{l s='Texte fabrication à la demande'}

							</div>
						</div>

						<div class="text_complementaire " id="livraison">
							<span data-toggle="collapse" data-target="#demo">{l s="Livraison sous 48h"}</span>
...
{if (isset($quantity_discounts) && count($quantity_discounts) > 0)}
		...
		{if isset($product) && $product->description}
			...
			<section class="page-product">
				<h2 align="center">{l s='Résumé produit'}</h2>
				<!-- full description -->
		...
		{if isset($product) && $product->tags}
			<!-- More info -->
			<h3 align="center" class="quality">{l s='Qualités Produit'}</h3>

			<ul align="center" class="producttags">
    ...
    {/foreach}
		...
		{if (Module::isInstalled(featuresproductsheet)) && Module::isEnabled(featuresproductsheet)}
		{hook h='FeaturesSheetHook'}
		...
		{if isset($accessories) && $accessories}
			<!--Accessories -->
			<section class="page-product-box1">
				<h3 class="page-product-heading2">Push produits <span style="background-color:#6f9ba4;color:#ffffff;">complémentaires</span></h3>
				<div class="block products_block accessories-block clearfix">
					<div class="block_content">
						<ul id="bxslider_accessory" class="bxslider clearfix">
								...
								{if ($accessory.allow_oosp || $accessory.quantity_all_versions > 0 || $accessory.quantity > 0) && $accessory.available_for_order && !isset($restricted_country_mode)}
									...
									<li class="item product-box ajax_block_product{if $smarty.foreach.accessories_list.first} first_item{elseif $smarty.foreach.accessories_list.last} last_item{else} item{/if} product_accessories_description">
										<div class="product_desc">
											...
											</a><div class="functional-buttons clearfix">
